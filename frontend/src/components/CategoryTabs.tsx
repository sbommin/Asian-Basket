...
import {
  ...
  ShoppingCart, Milk, Flame, Leaf, Cookie, Pizza,
  Grape, Cherry, Bean, Utensils, UtensilsCrossed,
  ChefHat, CookingPot, Croissant, Candy, Wine,
  Beer, GlassWater, Droplets, Package, Box,
  Sparkles, Star, Tag,
  type LucideIcon,
} from "lucide-react";
...
const ICON_MAP: Record<string, LucideIcon> = {
  Fish, Beef, Egg,
  Carrot, Apple, Salad, Grape, Cherry, Bean, Leaf,
  Wheat, Package, Box,
  Milk,
  Soup, Sandwich, Pizza, Croissant, CookingPot,
  Utensils, UtensilsCrossed, ChefHat, Flame,
  Cookie, IceCream, Candy,
  Coffee, Wine, Beer, GlassWater,
  Droplets,
  ShoppingBasket, ShoppingCart, Sparkles, Star, Tag,
};
...
const CategoryTabs = ({ activeTab, setActiveTab }: CategoryTabsProps) => {
  ...
  return (
    <div className="bg-white sticky top-[60px] md:top-[132px] z-30 shadow-sm border-b border-border mb-8 overflow-x-auto scrollbar-hide">
      <div className="container mx-auto px-4">
        {/* ✅ gap-6 + min-w-max matches v1 layout exactly */}
        <div className="flex items-center gap-6 min-w-max">
          ...
          <button
            onClick={() => setActiveTab("all")}
            className={cn(
              // ✅ Always flex-row — icon left, label right
          ...
          {/* LOADING SKELETONS */}
          {loading && [1, 2, 3, 4].map((i) => <TabSkeleton key={i} />)}
